...
#words_list = ['my','name','happy','me','good morning','good','morning','night','bye','hi','school','eat','walk','talk','he','she']
words_list = []
tbl_ply = dynamodb.Table('playlist_database')
rps_ply = tbl_ply.scan()
for i in rps_ply['Items']:
        words_list.append(i['video_title'])

def lambda_handler(event, context):
    # TODO implement
    
    #sentenceToTranslate = "i amm mahesh"
    sentenceToTranslate = event["sentence"]
    final_outtptview = main(sentenceToTranslate)
    
    return {
        'statusCode': 200,
        'body': json.dumps(final_outtptview)
    }
    

words_list = words_list + [i.replace('-',' ') for i in words_list if '-' in i]
...
#for since, because types statements 
  ...
  if "DATE" in type_list:
    ...
    if any("WRB" in tknchild.tag_ for tknchild in tkn.children) == False:
      if date_i not in done_list:
        ...
        if ' ' in tkn.text:
          lemtxt = nlp(tkn.text)
          txt = " ".join([i.lemma_ for i in lemtxt])
          ISLTokens.append(ISLToken(txt, tkn.i, tkn.dep_, tkn.head.i, tkn.tag_,
                                    tkn.ent_type_, [child for child in tkn.children]))
        else:
          ISLTokens.append(ISLToken(tkn.lemma_, tkn.i, tkn.dep_, tkn.head.i, tkn.tag_,
                                    tkn.ent_type_, [child for child in tkn.children]))
        ...
        if not tkn.i in done_list:
          for attr_children in tkn.children:
            if not attr_children.i in done_list:
              ...
              elif ' ' in attr_children.text:
                lemtxt = nlp(attr_children.text)
                txt = " ".join([i.lemma_ for i in lemtxt])
                ISLTokens.append(ISLToken(txt, attr_children.i, attr_children.dep_, attr_children.head.i, attr_children.tag_,
                                    attr_children.ent_type_, [child for child in attr_children.children]))
              else:
    ...
    if rootchild.dep_ == "advmod" and rootchild.tag_ == "RB":
      if any("W" in str(root_gchild.tag_) for root_gchild in list(rootchild.children)) == False:
        if not rootchild.i in done_list:
          ...
          if ' ' in tkn.text:
            lemtxt = nlp(rootchild.text)
            txt = " ".join([i.lemma_ for i in lemtxt])
            ISLTokens.append(ISLToken(txt, rootchild.i, rootchild.dep_, rootchild.head.i, rootchild.tag_,
      ...
      if tkn.lemma_ not in ['be','to']:
        if not tkn.dep_ in ["aux", "punct", "neg","auxpass"]:
          ...
          if ' ' in tkn.text:
            lemtxt = nlp(tkn.text)
            txt = " ".join([i.lemma_ for i in lemtxt])
            ISLTokens.insert(ISLToken(txt, tkn.i, tkn.dep_, tkn.i, tkn.tag_,
                                      tkn.ent_type_, [child for child in tkn.children]))
          else:
            ISLTokens.insert(j , ISLToken(tkn.lemma_, tkn.i, tkn.dep_, tkn.i, tkn.tag_,
                                      tkn.ent_type_, [child for child in tkn.children]))
      ...
      if tkn.lemma_ not in ['be','to']:
        if not tkn.dep_ in ["aux", "punct", "neg","poss"]:
          ...
          if ' ' in tkn.text:
            ...
            txt = " ".join([i.lemma_ for i in lemtxt])
            #changed tkn.head.i
            ISLTokens.insert(j, ISLToken(txt, tkn.i, tkn.dep_, tkn.i, tkn.tag_,
                                      tkn.ent_type_, [child for child in tkn.children]))
          else:
            ISLTokens.insert(j , ISLToken(tkn.lemma_, tkn.i, tkn.dep_, tkn.i, tkn.tag_,
                                      tkn.ent_type_, [child for child in tkn.children]))
          #CHANGED HEAD SINCE ON SPLITTING IT CAUSED ISSUE
          #IN NECT VERSION, CHANGE THE WHOLE ISL TOKEN THING
...
'''
#Phrases are kept waiting for a while

phrases_dict = {}
phrase_file = open("phrases.txt", "r")
phrases = phrase_file.read().lower().split('\n')
phrases_vals = ['-phrase'+str(i) for i in range(len(phrases))]
phrases_dict = dict((phrases[val], phrases_vals[val]) for val in range(len(phrases)))
'''
#end = time.time()
#print(end - begin)
#Use while loop here to keep it running consistently
...
def find_syn(line_list, words_list):
      ...
      for m in range(word_set_count,0,-1):
        if flag!=1:
          for n in range(word_set_count+1-m,0,-1):
            #print(n,m, n-1,n-1+m, word_set[n-1:n-1+m]," ".join(word_set[n-1:n-1+m]))

            if (" ".join(word_set[n-1:n-1+m])) in words_list:
              #print("yes")
              #main_list = main_list[:i] + word_set[:n] +  word_set[n:n+m] + word_set[n+m:] + main_list[i+1:]
              new_list = new_list + word_set[:n-1] + [" ".join(word_set[n-1:n-1+m])]+ word_set[n+m-1:]
              #new_list = new_list + word_set[:n-1] +  word_set[n-1:n-1+m] + word_set[n+m-1:] 
              #print("----",new_list,word_set[:n-1],word_set[n-1:n-1+m], word_set[n+m-1:])                 
              word_set_count = len(word_set)
              flag = 1
              break    
  ...
  for word_spot in range(len(main_list)):
    #print(main_sent[word_spot].tag_,text)
    if main_list[word_spot] in ['a', 'an' ,'the', 'and','of','at','or','for','by','in']:
      continue
    ...
    #But already there in ISL list so go ahead
      ...
      if main_list[word_spot] in words_list:
        ...
        token_synsets = wn.synsets(check_list[word_spot])
...
'''
#Loading of csv file and cleaning it
df_test = pd.read_csv('730sentences.csv')
ffi = []
df_test.dropna(subset = ["English"], inplace=True)
eng_sent = list(df_test['English'])
'''
...
def main(text):
    ...
    if not text: 
        text = "My name is Mahesh"
        
    ##return token_list(text)
    return final_output(input_split, translate_text, text ,token_list)
