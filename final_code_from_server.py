...
tbl_male = dynamodb.Table('playlist_database')
rps_male = tbl_male.scan()
for i in rps_male['Items']:
        words_list.append(i['video_title'])
for i in rps_male['Items']:
        video_list.append(i['video_key'])
...
# Adding indian names
names = []
tbl_male = dynamodb.Table('male_database')
rps_male = tbl_male.scan()
for i in rps_male['Items']:
        names.append(i['male_title'])

tbl_female = dynamodb.Table('female_database')
rps_female = tbl_female.scan()
for i in rps_female['Items']:
        names.append(i['female_title'])
...
def labeller(doc):
    ...
    matches = add_name(doc, 'i m balwant', names)
    new_ents = [Span(doc, match[1], match[2], label=PERSON) for match in matches if any(t.ent_type for t in doc[
                                                                                                            match[1]:
                                                                                                            match[
                                                                                                                2]]) == False]  # match[1] contains the start index of the the token and match[2] the stop index (exclusive) of the token in the
...
def eng_isl_translate(doc):  # , box_list):
                    ...
                    if not pobj_child.i in done_list:
                        if pobj_child.dep_ == "poss":
                            ...
                            if pobj_child.lemma_ == '-PRON-':
                                ISLTokens.append(
                                    ISLToken(pobj_child.text, pobj_child.i, pobj_child.dep_, pobj_child.head.i,
                                             pobj_child.tag_,
                                             pobj_child.ent_type_, [child for child in pobj_child.children]))
                            else:
                                ISLTokens.append(
                                    ISLToken(pobj_child.lemma_, pobj_child.i, pobj_child.dep_, pobj_child.head.i,
                                             pobj_child.tag_,
                                             pobj_child.ent_type_, [child for child in pobj_child.children]))
                        ...
                        if pobj_child.dep_ == "amod":
                            ...
                            ISLTokens.append(ISLToken(pobj_child.text, pobj_child.i, pobj_child.dep_, pobj_child.head.i,
                                                      pobj_child.tag_,
                                                      pobj_child.ent_type_, [child for child in pobj_child.children]))
                ...
                for attr_children in tkn.children:
                    if not attr_children.i in done_list:
                        ...
                        if attr_children.lemma_ == '-PRON-':
                            ISLTokens.append(
                                ISLToken(attr_children.text, attr_children.i, attr_children.dep_, attr_children.head.i,
                                         attr_children.tag_,
                                         attr_children.ent_type_, [child for child in attr_children.children]))
                        else:
                            ISLTokens.append(ISLToken(attr_children.lemma_, attr_children.i, attr_children.dep_,
                                                      attr_children.head.i, attr_children.tag_,
                                                      attr_children.ent_type_,
                                                      [child for child in attr_children.children]))
    ...
    if "dobj" in dep_list:
        ...
        if not dobj_i in done_list:
            ...
            for dobjchild in tkn.children:
                if dobjchild.dep_ in ["poss", "amod", "advcl", "det"] and dobjchild.text.lower() not in ['a', 'an',
                                                                                                         'the']:
                    if not dobjchild.i in done_list:
                        dobjchild_i = dep_list.index(dobjchild.dep_)
                        done_list.append(dobjchild_i)
                        ISLTokens.append(
                            ISLToken(dobjchild.text, dobjchild.i, dobjchild.dep_, dobjchild.head.i, dobjchild.tag_,
                ...
                if any("W" in str(nsubj_gchild.tag_) for nsubj_gchild in list(nsubj_child.children)) == False:
                    if nsubj_child.dep_ == "poss":
                        if not nsubj_child.i in done_list:
                            ...
                            if nsubj_child.lemma_ == '-PRON-':
                                ISLTokens.append(
                                    ISLToken(nsubj_child.text, nsubj_child.i, nsubj_child.dep_, nsubj_child.head.i,
                                             nsubj_child.tag_,
                                             nsubj_child.ent_type_, [child for child in nsubj_child.children]))
                            else:
                                ISLTokens.append(
                                    ISLToken(nsubj_child.lemma_, nsubj_child.i, nsubj_child.dep_, nsubj_child.head.i,
                                             nsubj_child.tag_,
                                             nsubj_child.ent_type_, [child for child in nsubj_child.children]))
        ...
        if rootchild.dep_ == "advmod" and rootchild.tag_ == "RB":
            if any("W" in str(root_gchild.tag_) for root_gchild in list(rootchild.children)) == False:
                if not rootchild.i in done_list:
                    ...
                    ISLTokens.append(
                        ISLToken(rootchild.lemma_, rootchild.i, rootchild.dep_, rootchild.head.i, rootchild.tag_,
    ...
    if "aux" in [child.dep_ for child in doc[root_i].children]:
        ...
        if root_children[aux_i_1].lemma_.lower() not in ['be', 'do', 'to', 'should', 'will', 'can', 'may', 'could',
                                                         'would']:
            tkn = doc[aux_i]
    ...
    if tag_list[0][0] == 'W':
        ...
        if token.head.dep_ == "amod" or token.head.dep_ == "advmod" or token.head.dep_ == "acomp":
            if not token.head.i in done_list:
                ...
                ISLTokens.append(
                    ISLToken(token.head.lemma_, token.head.i, token.head.dep_, token.head.head.i, token.head.tag_,
